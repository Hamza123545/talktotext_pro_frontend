    ...
    if (!token) {
        ...
        const API_URL = "https://talktotext-pro-backend.vercel.app";
        const token = localStorage.getItem("token");
        let historyData = [];
        let selectedHistoryIds = [];
        let currentHistoryItem = null;
        const user = JSON.parse(localStorage.getItem("user"));
        let userId = user ? (user.user_id || user._id || user.id) : null;
        ...
        function loadUserData() {
            fetch(`${API_URL}/auth/profile`, {
                ...
                .then(data => {
                    const prof = data.user || data; // backend returns user object at root
                    if (prof) {
                        document.getElementById('u_name').textContent = `${prof.firstname || ''} ${prof.lastname || ''}`.trim();
                        document.getElementById('u_role').textContent = prof.email || '';
                        document.getElementById('u_avatar').textContent = `${prof.firstname?.[0] || ''}${prof.lastname?.[0] || ''}`.toUpperCase();

                        // Ensure userId is set from profile if missing/mismatched
                        if (!userId) {
                            userId = prof._id || prof.user_id;
                        }
        ...
        function loadHistory(userId) {
            ...
            fetch(`${API_URL}/history?user_id=${userId}`, {
                ...
                .then(data => {
                    // Backend may return an array or an object { history_record: [...] }
